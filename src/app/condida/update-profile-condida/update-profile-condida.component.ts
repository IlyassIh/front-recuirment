...
@Component({
  selector: 'app-update-profile-condida',
  standalone: false,
  templateUrl: './update-profile-condida.component.html',
  styleUrl: './update-profile-condida.component.scss'
})
export class UpdateProfileCondidaComponent {
  hide1 = signal(true);
  ...
  updateProfile() {
    ...
    this.adminService.updateProfile(this.profileForm.value).subscribe({
      next: res => {
        ...
        this.userUpdate()
        this.router.navigate(['condida/home']);
      },
      error: err => {
        console.error(err);
